    ...
    <>
      <style>{`
        ...
        .tc-card.is-completed:hover {
          ...
          box-shadow: 0 8px 40px rgba(0,0,0,0.5), 0 0 0 1px rgba(34,197,94,0.1), 0 1px 0 rgba(34,197,94,0.15) inset;
        }
        .tc-header {
        ...
        .tc-issue-key {
          ...
          letter-spacing: 0.04em; text-shadow: 0 0 12px rgba(251,191,36,0.3);
        }
        .tc-child-badge {
        ...
        .tc-esc-row {
          padding: 7px 16px; border-bottom: 1px solid rgba(139,92,246,0.08);
        ...
        .tc-field-value.mono { font-family: 'JetBrains Mono', monospace; }
        ...
        .tc-show-more {
          margin-top: 4px; display: block; font-family: 'JetBrains Mono', monospace;
          font-size: 0.6rem; color: #a78bfa; background: none; border: none;
        ...
        .tc-status-label {
          font-family: 'JetBrains Mono', monospace; font-size: 0.58rem; color: #64748b;
          text-transform: uppercase; letter-spacing: 0.1em; white-space: nowrap;
        }
        .tc-select {
          background: rgba(21,27,43,0.9); border: 1px solid rgba(139,92,246,0.25);
          color: #e2e8f0; border-radius: 10px; padding: 5px 10px;
        ...
        .tc-app-badge {
          ...
          background: rgba(59,130,246,0.10); border: 1px solid rgba(59,130,246,0.25); color: #93c5fd;
          max-width: none; overflow: visible; text-overflow: unset; white-space: normal; cursor: default;
        }
        .tc-highlight {
          outline: 2px solid #facc15; box-shadow: 0 0 0 4px rgba(250,204,21,0.25);
      ...
      >
          ...
          >
            {esc ? (
              <>
                ...
                {!escIsAuto && !escIsHuman && !escIsQuarantine && (
                  <span className="tc-esc-badge escalated">🚀 Escalated</span>
                )}
                {escTeam && (
                  <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: "0.6rem", color: "#94a3b8" }}>
                    → <strong style={{ color: "#c4b5fd" }}>{escTeam}</strong>
                    {escLevel && (
                      <span style={{ marginLeft: 4, fontWeight: 700, color: escLevel === "L3" ? "#f87171" : escLevel === "L2" ? "#60a5fa" : "#facc15" }}>
                        {escLevel}
                      </span>
                    )}
                  </span>
                ...
                {escNeedsReview && (
                  <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: "0.58rem", color: "#facc15", background: "rgba(250,204,21,0.08)", border: "1px solid rgba(250,204,21,0.2)", padding: "2px 8px", borderRadius: 999 }}>
                    ◆ Review Needed
                  </span>
                )}
              </>
            ) : (
              <span className="tc-esc-badge l1">● Status: Assigned at L1</span>
            )}
          </div>
        )}
        ...
        <div className="tc-body">
          ...
          {/* Priority + SLA — only shown when present */}
          {t.priority && (
          ...
          <button
            ...
            onClick={() => {
              if (t.parent_ticket_key) {
                alert("Merge allowed only from parent tickets");
                return;
              }
              onOpenMerge(t.issue_key);
            }}
          >
            🔀 Merge
          </button>
        ...
        <div className="tc-actions row2">
          {/* P1/P2 HITL review button */}
          {t.priority && ["P1", "P2"].includes(t.priority) && t.hitl_reviewed !== true && !isCompleted && (
            <button
              className="tc-btn"
              style={{ background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.3)", color: "#fca5a5", boxShadow: "0 2px 8px rgba(239,68,68,0.1)" }}
          ...
          {/* Re-assess SLA button */}
          {onReassessPrioritySla && !isCompleted && t.impact && t.urgency && (
            <button
              className="tc-btn"
              style={{ background: "rgba(59,130,246,0.12)", border: "1px solid rgba(59,130,246,0.3)", color: "#93c5fd", boxShadow: "0 2px 8px rgba(59,130,246,0.08)" }}
              onMouseEnter={e => { e.currentTarget.style.background = "rgba(59,130,246,0.18)"; }}
              onMouseLeave={e => { e.currentTarget.style.background = "rgba(59,130,246,0.12)"; }}
              onClick={() => onReassessPrioritySla({
          ...
          {!isCompleted && (
            ...
            >
              {escIsAuto      ? "✦ Auto-Escalated"
               : escIsHuman  ? "✔ Reviewed"
               : escIsQuarantine ? "⚠ Quarantine"
               : escNeedsReview  ? "◆ Review Escalation"
               : escTeam         ? "🚀 View Escalation"
               :                   "🚀 Escalate"}
            </button>
          )}
        </div>
